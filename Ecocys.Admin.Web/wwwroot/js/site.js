...
$(document).on("input", ".js-only-number", function () {
    $(this).val($(this).val().replace(/[^0-9]/g, ""));
});

$(document).on("input", ".js-only-lat-long", function () {
    var value = $(this).val();
    value = value.replace(/[^0-9-+.]/g, '');
    if (value.split('.').length > 2) value = value.substring(0, value.length - 1);//need to do same for + and - to allow only single.
    $(this).val(value);
});
...
function ShowHidePassword(span) {
    let elePassword = $('#Password');
    if (elePassword.attr('type') == 'password') {
        elePassword.attr('type', 'text');
        $(span).find('i').attr('class', 'fa fa-eye-slash');
    } else {
        elePassword.attr('type', 'password');
        $(span).find('i').attr('class', 'fa fa-eye');
    }
}